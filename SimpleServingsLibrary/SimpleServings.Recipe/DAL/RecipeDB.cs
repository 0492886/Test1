...
namespace SimpleServingsLibrary.Recipe
{
    class RecipeDB
    {
        ...
        internal static SqlDataReader GetRecipesforTagsSearch(List<int> TagIDs, string RecipeSearchText)
        {
            Database db = DatabaseFactory.CreateDatabase(GlobalEnum.DATABASENAME);
            DbCommand cmd = db.GetStoredProcCommand("SP_SearchRecipesForTags");
        ...
        {
            Database db = DatabaseFactory.CreateDatabase(GlobalEnum.DATABASENAME);
            DbCommand cmd = db.GetStoredProcCommand("Sp_Get_RecipesByStaffIDAndRecipeID_test");
            db.AddInParameter(cmd, "@recipeID", DbType.Int32, recipeID);
            db.AddInParameter(cmd, "@staffID", DbType.Int32, staffID);
            SqlDataReader dr = (SqlDataReader)db.ExecuteReader(cmd);
            if (dr.HasRows)
