...
namespace SimpleServingsLibrary.Menu
{
    class MenuDB
    {
        ...
        internal static SqlDataReader GetMyMenusByUserIDContractIDpeMenuStatus(int staffID, int contractID, int menuStatus, int currentMenus)
        {
            Database db = DatabaseFactory.CreateDatabase(GlobalEnum.DATABASENAME);
            DbCommand cmd = db.GetStoredProcCommand("Sp_Get_MyMenus_by_UserID_contractID_menustatus");
            db.AddInParameter(cmd, "@staffID", DbType.Int32, staffID);
