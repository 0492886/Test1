...
<asp:GridView CssClass="DatagridStyle table table-hover" GridLines="None" 
        ...
        onpageindexchanging="gvMenus_PageIndexChanging" onsorting="gvMenus_Sorting">
...
<Columns>
    ...
    <asp:TemplateField HeaderText="Name" SortExpression="MenuName">
     <ItemTemplate>
            <asp:Label ID="lblMenuName" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "MenuName") %>' ></asp:Label>
     </ItemTemplate>
    </asp:TemplateField>
   
   <asp:TemplateField HeaderText="Program Type" SortExpression="ContractType">
     <ItemTemplate>
            <asp:Label ID="lblContractType" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "ContractTypeName") %>'  Width="125px"></asp:Label>
     </ItemTemplate>
    </asp:TemplateField>
    ...
    <asp:HyperLinkField DataNavigateUrlFields="MenuID" ControlStyle-CssClass="" DataNavigateUrlFormatString="~/UI/Page/SimpleServings/Menu/ViewMenu.aspx?MenuID={0}" Text="View"/>  <%--Target="_blank"--%> 
        ...
        <asp:TemplateField>
            ...
            <asp:LinkButton ID="lnkBtnRemove" runat="server" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "MenuID") %>'
                    OnClick="lnkBtnRemove_Click">Delete</asp:LinkButton> <%--Visible='<%# Request.QueryString.Get("MenuType") == "MyMenus"? true:false %>'--%>
            <cc1:ConfirmButtonExtender ID="ConfirmButtonExtender1" runat="server" ConfirmText="Are you sure you want to delete the menu? If so, please click OK button" TargetControlID="lnkBtnRemove">
